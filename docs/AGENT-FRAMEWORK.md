...
|---|---|---|---|
| Never commit secrets or live tokens | A live 128-char production token (`LOOM_TABLE_EXPLORER_TOKEN`) was hardcoded in `apps/storefront/src/app/api/backend/[...path]/route.ts` and pushed to `main` in a merged PR. It has been in git history since that merge. Rotation on the Loom side, not just removal, is required. | **ENFORCED** | The `secret-scan` job in `.github/workflows/ci.yml` runs gitleaks over full history and fails the build. Note this backstop was added *after* the incident: the token is already in history and cannot be rotated (it is a shared credential), so it is recorded as accepted exposure in `docs/KNOWN-GAPS.md`. The scanner prevents the next one. |
...
- Secret scanning now runs (gitleaks, `.github/workflows/ci.yml`). It did not exist when the live token in §3 reached `main`, which is how that happened.
