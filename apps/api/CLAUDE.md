...
| `auth/` | Password auth (`GatekeeperService`, scrypt), tenant lookup, `RolesGuard`. Auth0/social login bound to a dummy that always rejects. | **Real.** This is where auth actually lives today, not `identity/`. |
