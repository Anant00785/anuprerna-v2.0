...
export class OrderFeedbackController {
  ...
  async addFeedback(@Body() body: any) {
    try {
      ...
      await this.db.insert(schema.purchaseOrderFeedback).values({
        orderId,
        question1: body?.question1 || "Rate Your Experience",
        question1Answer: String(body?.question1Answer || "5"),
        question2: body?.question2 || "Did you find what you were looking for?",
        question2Answer: Boolean(body?.question2Answer ?? true),
        question2Negative: body?.question2Negative || "",
        question2NegAnswer: body?.question2NegAnswer || "",
        question3: body?.question3 || "Any additional suggestions?",
  ...
  async updateFeedbackQ1(@Body() body: any) {
    try {
      const id = BigInt(body?.id || body?.feedbackId || 0);
      await this.db.update(schema.purchaseOrderFeedback).set({
        question1Answer: String(body?.rating || body?.answer || "5"),
        updatedAt: Date.now(),
  ...
  async updateFeedbackQ3(@Body() body: any) {
    try {
      const id = BigInt(body?.id || body?.feedbackId || 0);
      await this.db.update(schema.purchaseOrderFeedback).set({
        question3Answer: body?.answer || body?.comment || "",
        updatedAt: Date.now(),
      }).where(eq(schema.purchaseOrderFeedback.id, id));
      return simpleResponse(true, "Feedback updated successfully.");
    } catch {
  ...
  async getFeedbackByOrder(@Param("orderId") orderId: string) {
    ...
    return keyedResponse("feedback", rows[0] || null);
  }
  ...
  async getFeedbackList() {
    ...
    return keyedResponse("feedbackList", rows);
  }
}
