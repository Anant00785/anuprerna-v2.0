...
describe("SearchController#searchProduct (legacy /get/search/result/:keyword)", () => {
  ...
  it("swallows a service error into an empty productPreviewList rather than propagating it", async () => {
    const controller = makeController({
      searchProduct: async () => {
    ...
    const res = await controller.searchProduct("silk");
    expect(res).toEqual({ success: true, message: "", productPreviewList: [] });
  });
});
...
describe("SearchController#aiSearch (/search/ai/:keyword)", () => {
  it("validates, delegates to searchService.searchProductV2, and swallows errors into the legacy empty searchResult shape", async () => {
    ...
    expect(await throwingController.aiSearch("cotton")).toEqual({ success: true, message: "", searchResult: { product: { resultSet: [], relatedResultSet: [] } } });
  });
});
