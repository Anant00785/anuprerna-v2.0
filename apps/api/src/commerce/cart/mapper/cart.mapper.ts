...
export function toInsertValues(tenantId: number, input: AddCartItemRequest): InsertCartItemValues {
  return {
    ...
    fabricProductId: input.fabricProductId && Number(input.fabricProductId) > 0 ? Number(input.fabricProductId) : null,
    finishedProductId: input.finishedProductId && Number(input.finishedProductId) > 0 ? Number(input.finishedProductId) : null,
    selectedFabricId: input.selectedFabricId && Number(input.selectedFabricId) > 0 ? Number(input.selectedFabricId) : null,
    selectedSizeOptionId: input.selectedSizeOptionId && Number(input.selectedSizeOptionId) > 0 ? Number(input.selectedSizeOptionId) : null,
    ...
    makingCharge: String(input.makingCharge ?? "0.00"),
