...
import { ProfileService, SizeProfile, ProfileSizeConfigItem } from '@/services/profile-service';
...
export default function SizeProfilePage() {
  ...
  const [editingProfile, setEditingProfile] = useState<SizeProfile | null>(null);
  ...
  const loadData = async () => {
    ...
    setError('');
    try {
      const data = await ProfileService.getSizeProfiles();
      setProfiles(data);
    } catch (err: any) {
      setError(err.message || 'Failed to load size profiles.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenAddModal = () => {
    ...
    setFormData({
      ...
      disclaimer: '',
      image: '',
      items: [{ label: 'S', keyFeature: 'Small', consumedFabric: 1.5, sortOrder: 1 }],
    });
    setIsModalOpen(true);
  };

  const handleOpenEditModal = async (profile: SizeProfile) => {
    setEditingProfile(profile);
    setIsModalOpen(true);
    try {
      if (profile.id) {
        const fullDetail = await ProfileService.getSizeProfileById(profile.id);
        setFormData({
          profileName: fullDetail.profileName || '',
          displayName: fullDetail.displayName || 'Sizes',
          disclaimer: fullDetail.disclaimer || '',
          image: fullDetail.image || '',
          items: fullDetail.sizeProfileOptionList && fullDetail.sizeProfileOptionList.length > 0
            ? fullDetail.sizeProfileOptionList
            : [{ label: 'S', keyFeature: 'Small', consumedFabric: 1.5, sortOrder: 1 }],
        });
      }
    } catch {
      setFormData({
        profileName: profile.profileName || '',
        displayName: profile.displayName || 'Sizes',
        disclaimer: profile.disclaimer || '',
        image: profile.image || '',
        items: profile.sizeProfileOptionList && profile.sizeProfileOptionList.length > 0
          ? profile.sizeProfileOptionList
          : [{ label: 'S', keyFeature: 'Small', consumedFabric: 1.5, sortOrder: 1 }],
      });
    }
  };
  ...
  const handleAddItem = () => {
    setFormData(prev => ({
      ...
      items: [
        ...prev.items,
        { label: '', keyFeature: '', consumedFabric: 1, sortOrder: prev.items.length + 1 },
      ],
    }));
  };
  ...
  const handleItemChange = (index: number, field: keyof ProfileSizeConfigItem, value: any) => {
    setFormData(prev => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const payload: SizeProfile = {
        profileName: formData.profileName.trim(),
        displayName: formData.displayName.trim(),
        disclaimer: formData.disclaimer.trim(),
        image: formData.image.trim(),
        sizeProfileOptionList: formData.items,
      };

      if (editingProfile && editingProfile.id) {
        await ProfileService.updateSizeProfile(editingProfile.id, { ...payload, id: editingProfile.id });
      } else {
        await ProfileService.addSizeProfile(payload);
      }

      setIsModalOpen(false);
      loadData();
    } catch (err: any) {
      alert(err.message || 'Failed to save size profile.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await ProfileService.deleteSizeProfile(deletingId);
      setDeletingId(null);
      loadData();
    } catch (err: any) {
      alert(err.message || 'Failed to delete size profile.');
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">Size Profiles</h1>
            <p className="text-xs text-slate-500">Standard size tables and size guide options shown on product pages</p>
          </div>
        </div>
      ...
      <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3">
        ...
        <input
          type="text"
          placeholder="Search size profiles..."
          value={searchTerm}
      ...
      {loading ? (
        <div className="bg-white p-12 rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm text-slate-500 font-medium">Loading size profiles...</p>
        </div>
      ) : (
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs font-semibold text-indigo-600">{p.displayName || 'Sizes'}</td>
                    <td className="px-6 py-4 text-xs text-slate-600">{(p.sizeProfileOptionList || []).length} option(s)</td>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 max-w-2xl w-full p-6 shadow-xl space-y-6 my-8">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <h2 className="text-lg font-bold text-slate-900">
                {editingProfile ? 'Edit Size Profile' : 'Add Size Profile'}
              </h2>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Standard Apparel Size Chart"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">
                    Size Guide Chart Image URL
                  </label>
                  <input
                    type="text"
                    value={formData.image}
                    onChange={e => setFormData({ ...formData, image: e.target.value })}
                    placeholder="e.g. https://domain.com/size-guide.png"
                    className="w-full px-3.5 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-slate-900"
                  />
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold text-slate-900">Size Options</h3>
                  <button
                    type="button"
                  ...
                  {formData.items.map((item, idx) => (
                    <div key={idx} className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-3 relative">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                        ...
                        <input
                          type="text"
                          placeholder="Feature (e.g. Chest 38)"
                          value={item.keyFeature || ''}
                          onChange={e => handleItemChange(idx, 'keyFeature', e.target.value)}
                          className="px-3 py-1.5 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900"
                        />
                        <input
                          type="number"
                          ...
                          value={item.consumedFabric || 0}
                          onChange={e => handleItemChange(idx, 'consumedFabric', parseFloat(e.target.value) || 0)}
                          className="px-3 py-1.5 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900"
                        />
                        <input
                          type="number"
                          ...
                          onChange={e => handleItemChange(idx, 'sortOrder', parseInt(e.target.value) || idx + 1)}
                          className="px-3 py-1.5 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900"
                        />
                      </div>
                    </div>
      ...
      {deletingId && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-200 max-w-sm w-full p-6 shadow-xl space-y-4">
            <h3 className="text-base font-bold text-slate-900">Delete Size Profile</h3>
            <p className="text-xs text-slate-500">
              Are you sure you want to delete this size profile? This action cannot be undone.
            </p>
