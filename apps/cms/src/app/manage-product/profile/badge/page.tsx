...
import { ProfileService, BadgeProfile, ProfileBadgeConfigItem } from '@/services/profile-service';

export default function BadgeProfilePage() {
  ...
  const [editingProfile, setEditingProfile] = useState<BadgeProfile | null>(null);
  ...
  const loadData = async () => {
    ...
    setError('');
    try {
      const data = await ProfileService.getBadgeProfiles();
      setProfiles(data);
    } catch (err: any) {
      setError(err.message || 'Failed to load badge profiles.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenAddModal = () => {
    ...
    setFormData({
      profileName: '',
      items: [{ caption: '', link: '', image: '' }],
    });
    setIsModalOpen(true);
  };

  const handleOpenEditModal = async (profile: BadgeProfile) => {
    setEditingProfile(profile);
    setIsModalOpen(true);
    try {
      if (profile.id) {
        const fullDetail = await ProfileService.getBadgeProfileById(profile.id);
        setFormData({
          profileName: fullDetail.profileName || '',
          items: fullDetail.badgeProfileItemList && fullDetail.badgeProfileItemList.length > 0
            ? fullDetail.badgeProfileItemList
            : [{ caption: '', link: '', image: '' }],
        });
      }
    } catch {
      setFormData({
        ...
        items: profile.badgeProfileItemList && profile.badgeProfileItemList.length > 0
          ? profile.badgeProfileItemList
          : [{ caption: '', link: '', image: '' }],
      });
    }
  };
  ...
  const handleAddItem = () => {
    setFormData(prev => ({
      ...prev,
      items: [...prev.items, { caption: '', link: '', image: '' }],
    }));
  };
  ...
  const handleItemChange = (index: number, field: keyof ProfileBadgeConfigItem, value: any) => {
    setFormData(prev => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const payload: BadgeProfile = {
        profileName: formData.profileName.trim(),
        badgeProfileItemList: formData.items,
      };

      if (editingProfile && editingProfile.id) {
        await ProfileService.updateBadgeProfile(editingProfile.id, { ...payload, id: editingProfile.id });
      } else {
        await ProfileService.addBadgeProfile(payload);
      }

      setIsModalOpen(false);
      loadData();
    } catch (err: any) {
      alert(err.message || 'Failed to save badge profile.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await ProfileService.deleteBadgeProfile(deletingId);
      setDeletingId(null);
      loadData();
    } catch (err: any) {
      alert(err.message || 'Failed to delete badge profile.');
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">Badge Profiles</h1>
            <p className="text-xs text-slate-500">Display badges (Handwoven, Organic, Limited Edition) shown over product images</p>
          </div>
        </div>
      ...
      <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3">
        ...
        <input
          type="text"
          placeholder="Search badge profiles..."
          value={searchTerm}
      ...
      {loading ? (
        <div className="bg-white p-12 rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm text-slate-500 font-medium">Loading badge profiles...</p>
        </div>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-400 font-semibold uppercase text-xs tracking-wider">
              <tr>
                ...
                <th className="px-6 py-3.5">Badges Count</th>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs text-slate-600">{(p.badgeProfileItemList || []).length} badge(s)</td>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 max-w-2xl w-full p-6 shadow-xl space-y-6 my-8">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <h2 className="text-lg font-bold text-slate-900">
                {editingProfile ? 'Edit Badge Profile' : 'Add Badge Profile'}
              </h2>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Eco & Craft Badges"
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold text-slate-900">Badge Items</h3>
                  <button
                    type="button"
                ...
                <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
                  {formData.items.map((item, idx) => (
                    <div key={idx} className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-3 relative">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-slate-500">Badge #{idx + 1}</span>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <input
                          type="text"
                          placeholder="Caption (e.g. Handwoven)"
                          value={item.caption || ''}
      ...
      {deletingId && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-200 max-w-sm w-full p-6 shadow-xl space-y-4">
            <h3 className="text-base font-bold text-slate-900">Delete Badge Profile</h3>
            <p className="text-xs text-slate-500">
              Are you sure you want to delete this badge profile? This action cannot be undone.
            </p>
