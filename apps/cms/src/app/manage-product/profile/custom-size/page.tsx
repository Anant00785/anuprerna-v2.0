...
import { ProfileService, CustomSizeProfile, ProfileCustomSizeConfigItem } from '@/services/profile-service';

export default function CustomSizeProfilePage() {
  const [profiles, setProfiles] = useState<CustomSizeProfile[]>([]);
  ...
  const [editingProfile, setEditingProfile] = useState<CustomSizeProfile | null>(null);
  ...
  const [formData, setFormData] = useState<{
    ...
    price: number;
    disclaimer: string;
    items: ProfileCustomSizeConfigItem[];
  }>({
    profileName: '',
    price: 0,
    disclaimer: '',
  ...
  const loadData = async () => {
    ...
    setError('');
    try {
      const data = await ProfileService.getCustomSizeProfiles();
      setProfiles(data);
    } catch (err: any) {
      setError(err.message || 'Failed to load custom size profiles.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenAddModal = () => {
    ...
    setFormData({
      ...
      price: 0,
      disclaimer: '',
      items: [{ label: '', placeholder: '', fieldType: 0, mandatory: true }],
    });
    setIsModalOpen(true);
  };

  const handleOpenEditModal = async (profile: CustomSizeProfile) => {
    setEditingProfile(profile);
    setIsModalOpen(true);
    try {
      if (profile.id) {
        const fullDetail = await ProfileService.getCustomSizeProfileById(profile.id);
        setFormData({
          profileName: fullDetail.profileName || '',
          price: fullDetail.price || 0,
          disclaimer: fullDetail.disclaimer || '',
          items: fullDetail.customSizeProfileItemList && fullDetail.customSizeProfileItemList.length > 0
            ? fullDetail.customSizeProfileItemList
            : [{ label: '', placeholder: '', fieldType: 0, mandatory: true }],
        });
      }
    } catch {
      setFormData({
        ...
        items: profile.customSizeProfileItemList && profile.customSizeProfileItemList.length > 0
          ? profile.customSizeProfileItemList
          : [{ label: '', placeholder: '', fieldType: 0, mandatory: true }],
      });
    }
  };
  ...
  const handleAddItem = () => {
    setFormData(prev => ({
      ...prev,
      items: [...prev.items, { label: '', placeholder: '', fieldType: 0, mandatory: true }],
    }));
  };
  ...
  const handleItemChange = (index: number, field: keyof ProfileCustomSizeConfigItem, value: any) => {
    setFormData(prev => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const payload: CustomSizeProfile = {
        profileName: formData.profileName.trim(),
        price: formData.price,
        disclaimer: formData.disclaimer.trim(),
        customSizeProfileItemList: formData.items,
      };

      if (editingProfile && editingProfile.id) {
        await ProfileService.updateCustomSizeProfile({ ...payload, id: editingProfile.id });
      } else {
        await ProfileService.addCustomSizeProfile(payload);
      }

      setIsModalOpen(false);
      loadData();
    } catch (err: any) {
      alert(err.message || 'Failed to save custom size profile.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await ProfileService.deleteCustomSizeProfile(deletingId);
      setDeletingId(null);
      loadData();
    } catch (err: any) {
      alert(err.message || 'Failed to delete custom size profile.');
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">Custom Size Profiles</h1>
            <p className="text-xs text-slate-500">Rules that let buyers request custom sizing inputs & measurements</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>Add Custom Size Profile</span>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3">
        ...
        <input
          type="text"
          placeholder="Search custom size profiles..."
          value={searchTerm}
      ...
      {loading ? (
        <div className="bg-white p-12 rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm text-slate-500 font-medium">Loading custom size profiles...</p>
        </div>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-400 font-semibold uppercase text-xs tracking-wider">
              <tr>
                ...
                <th className="px-6 py-3.5">Fields Count</th>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs text-slate-600">{(p.customSizeProfileItemList || []).length} field(s)</td>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 max-w-2xl w-full p-6 shadow-xl space-y-6 my-8">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <h2 className="text-lg font-bold text-slate-900">
                {editingProfile ? 'Edit Custom Size Profile' : 'Add Custom Size Profile'}
              </h2>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Standard Garment Custom Sizing"
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. Custom sizes require additional processing time of 3-5 business days."
                  className="w-full px-3.5 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-slate-900"
                />
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold text-slate-900">Custom Input Fields</h3>
                  <button
                    type="button"
                  ...
                  {formData.items.map((item, idx) => (
                    <div key={idx} className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-3 relative">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-center">
                        <input
                          type="text"
                          placeholder="Field Label (e.g. Chest Inch)"
                          value={item.label || ''}
                        ...
                        <input
                          type="text"
                          placeholder="Placeholder (e.g. Enter chest size)"
                          value={item.placeholder || ''}
                          onChange={e => handleItemChange(idx, 'placeholder', e.target.value)}
                          className="px-3 py-1.5 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900"
                        />
                        <label className="flex items-center gap-2 text-xs font-semibold text-slate-700 cursor-pointer">
                          <input
                            ...
                            onChange={e => handleItemChange(idx, 'mandatory', e.target.checked)}
      ...
      {deletingId && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-200 max-w-sm w-full p-6 shadow-xl space-y-4">
            <h3 className="text-base font-bold text-slate-900">Delete Custom Size Profile</h3>
            <p className="text-xs text-slate-500">
              Are you sure you want to delete this custom size profile? This action cannot be undone.
            </p>
