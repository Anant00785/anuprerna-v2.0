...
import { ArrowLeft, Search, Plus, Loader2, RefreshCw, Edit2, Trash2, X, Check, Tag as TagIcon, Sparkles, AlertTriangle, Calendar } from 'lucide-react';
import { ProductService, SpecialStatus } from '@/services/product-service';
...
export default function SpecialStatusPage() {
  ...
  const loadData = async () => {
    ...
    setError('');
    try {
      const data = await ProductService.getSpecialStatuses();
      setStatuses(data);
    } catch (err: any) {
      setError(err.message || 'Failed to load special statuses.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredStatuses = statuses.filter(s => !searchTerm || (s.name || '').toLowerCase().includes(searchTerm.toLowerCase()));

  // Metrics
  const statsTotal = statuses.length;
  const statsNewThisMonth = statuses.filter(s => (s.timeOfCreation ?? 0) > Date.now() - RECENT_WINDOW_DAYS * MS_PER_DAY).length;
  const statsWithTrailingSpace = statuses.filter(s => {
    const n = s.name || '';
  ...
  const openEditModal = (s: SpecialStatus) => {
    setEditingId(s.id);
    setName(s.name || '');
    setModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      if (editingId) await ProductService.updateSpecialStatus(editingId, name);
      else await ProductService.createSpecialStatus(name);
      setModalOpen(false);
      loadData();
    } catch (err: any) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await ProductService.deleteSpecialStatus(deletingId);
      setDeletingId(null);
      loadData();
    } catch (err: any) {
      alert(err.message || 'Failed to delete special status');
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">Special Status</h1>
            <p className="text-xs text-slate-500 font-normal">
              Merchandising tags applied to products — badges, filter facets, and curated collection pages
            </p>
          </div>
        </div>
      ...
      <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3">
        ...
        <input
          type="text"
          placeholder="Search special statuses by name..."
          value={searchTerm}
      ...
      {loading ? (
        <div className="bg-white p-12 rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm text-slate-500 font-medium">Loading special status tags...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {filteredStatuses.map(s => (
            <div key={s.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between gap-4 group hover:border-slate-300 transition-all">
                ...
                <div className="flex items-center gap-1.5 text-[11px]">
                  ...
                  {s.timeOfCreation ? (
                    <span className="flex items-center gap-1">
                      ...
                      Created {relativeTime(s.timeOfCreation)} ago
                    </span>
                  ) : null}
                </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleSave} className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-md overflow-hidden p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="font-bold text-slate-900 text-base">{editingId ? 'Edit Special Status' : 'Add Special Status'}</h3>
            ...
            <div>
              <label className="block font-semibold text-slate-700 text-xs uppercase tracking-wider mb-1">Status Title *</label>
              <input type="text" required placeholder="e.g. Bestseller, New Arrival, Organic..." value={name} onChange={e => setName(e.target.value)} className="w-full px-3.5 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl outline-none focus:bg-white focus:ring-2 focus:ring-slate-900" />
            </div>
      ...
      {deletingId && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-200 max-w-sm w-full p-6 shadow-xl space-y-4">
            <h3 className="text-base font-bold text-slate-900">Delete Special Status</h3>
            <p className="text-xs text-slate-500">
              Are you sure you want to delete this special status? This action cannot be undone.
            </p>
