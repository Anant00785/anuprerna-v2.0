...
export default function ManageProcessPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      setError('');
      try {
        const response = await apiClient.get(`/get/workflow-list/${activeTab}`);
        const data = unwrapResponseData<WorkflowItem[]>(response.data, 'workflowList') || [];
        setItems(data);
        setCurrentPage(1);
      } catch (err: any) {
        setError(err.message || `Failed to fetch ${activeTab.toLowerCase()} workflow processes.`);
        setItems([]);
      } finally {
        setLoading(false);
      }
        ...
        {loading ? (
          <div className="min-h-[40vh] flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-xs text-slate-500 font-medium">
              Loading {activeTab.toLowerCase()} processes...
            </p>
          </div>
        ) : items.length === 0 ? (
          <div className="text-center text-slate-400 text-xs py-16">
            No {activeTab.toLowerCase()} workflow processes found.
          </div>
        ) : (
