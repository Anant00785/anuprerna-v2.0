...
export default function AddInventoryReasonPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setApiError(err.message || 'Failed to add inventory reason');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl space-y-6 pb-12">
      <div className="flex items-center gap-3">
        ...
        <div>
          <PageHeading heading="Add Inventory Audit Reason" />
          <p className="text-xs text-slate-500 font-normal mt-0.5">
            Create a new reason code for inventory adjustments
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-5">
        <div>
          ...
          <input
            type="text"
            placeholder="e.g. Sampling Cut, Damage in Transit, Weaving Quality Defect"
            value={reason}
        ...
        <div>
          ...
          <textarea
            rows={4}
            placeholder="e.g. Audit reason used during loom fabric inspection failures."
            value={description}
