...
export default function UpdateInventoryReasonPage() {
  ...
  const loadReason = async (id: number) => {
    setLoading(true);
    try {
      const r = await InventoryService.getInventoryReasonById(id);
      if (r) {
        setReason(r.reason || '');
        setDescription(r.description || '');
      }
    } catch (err: any) {
      setApiError(err.message || 'Failed to fetch inventory reason details.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const validate = () => {
    const err: { reason?: string; description?: string } = {};
    if (!reason.trim()) err.reason = 'Reason code is required';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/inventory/inventory-reason');
    } catch (err: any) {
      setApiError(err.message || 'Failed to update inventory reason');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="max-w-2xl bg-white p-12 rounded-3xl border border-slate-200 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm text-slate-500 font-medium">Loading inventory reason...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl space-y-6 pb-12">
      <div className="flex items-center gap-3">
        <Link
          href="/inventory/inventory-reason"
          className="p-2 text-slate-500 hover:text-slate-900 rounded-xl hover:bg-slate-100 transition-all"
        >
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <PageHeading heading={`Edit Audit Reason #${reasonId}`} />
          <p className="text-xs text-slate-500 font-normal mt-0.5">
            Modify inventory adjustment reason code & description
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-5">
        <div>
          ...
          <input
            type="text"
            placeholder="e.g. Sampling Cut, Damage in Transit"
            value={reason}
