...
export default async function Page({ params }: PageProps) {
  ...
  // reflects the true /b2b/craft-color/<slug> hierarchy (middle level has no
  ...
  const path = `/b2b/craft-color/${slug}`;
  const breadcrumbJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: absUrl('/') },
      { '@type': 'ListItem', position: 2, name: 'Craft Color' },
  ...
  return (
    <>
      ...
      <B2bLandingPage slug={slug} variant="craft-color" products={products} />
    </>
  );
}
