...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: `Source premium handwoven ${title.toLowerCase()} in bulk. Flexible MOQs, custom dyeing, and global delivery from Anuprerna artisan fabric suppliers.`,
  };
}
...
export default async function Page({ params }: PageProps) {
  ...
  // reflects the true /b2b/fabric-pattern/<slug> hierarchy (middle level has no
  ...
  const path = `/b2b/fabric-pattern/${slug}`;
  const breadcrumbJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: absUrl('/') },
      { '@type': 'ListItem', position: 2, name: 'Fabric Pattern' },
  ...
  return (
    <>
      ...
      <B2bLandingPage slug={slug} variant="fabric-pattern" products={products} />
    </>
  );
}
