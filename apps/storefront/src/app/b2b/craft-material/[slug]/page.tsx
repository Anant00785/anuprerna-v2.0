...
export default async function Page({ params }: PageProps) {
  ...
  // reflects the true /b2b/craft-material/<slug> hierarchy (middle level has no
  ...
  const path = `/b2b/craft-material/${slug}`;
  const breadcrumbJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: absUrl('/') },
      { '@type': 'ListItem', position: 2, name: 'Craft Material' },
  ...
  return (
    <>
      ...
      <B2bLandingPage slug={slug} variant="craft-material" products={products} />
    </>
  );
}
