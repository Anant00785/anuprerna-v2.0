...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: `Source premium handwoven ${title.toLowerCase()} in bulk. Flexible MOQs, custom orders, and global delivery from Anuprerna artisan suppliers.`,
  };
}
...
export default async function Page({ params }: PageProps) {
  ...
  // reflects the true /b2b/pattern-material/<slug> hierarchy (middle level has no
  ...
  const path = `/b2b/pattern-material/${slug}`;
  const breadcrumbJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: absUrl('/') },
      { '@type': 'ListItem', position: 2, name: 'Pattern Material' },
  ...
  return (
    <>
      ...
      <B2bLandingPage slug={slug} variant="pattern-material" products={products} />
    </>
  );
}
