        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredBlogs.map((blog, idx) => {
              ...
              const imageUrl =
                blog.heroImage || blog.bannerImageMobile || blog.bannerImage || "https://images.unsplash.com/photo-1606744888344-493238951221?auto=format&fit=crop&w=800&q=80";

              return (
                <Link
                  key={blogId || idx}
                  href={`/content/${blog.contentType || "wholesale"}/${blog.slug || "article"}/${blogId}`}
                ...
                >
                  ...
                  <div className="p-6 flex flex-col justify-between flex-1 gap-3">
                    <div>
                      <h2 className="font-serif font-bold text-lg text-gray-900 leading-snug group-hover:text-[#8E7862] transition-colors line-clamp-2">
                        {blog.title}
                      </h2>
                      {blog.description && (
